...
Everything else (`SECRET_KEY`, `CREDENTIAL_ENCRYPTION_KEY`, `INTERNAL_API_SECRET`, `REDIS_PASSWORD`, `REDIS_BACKEND_PASSWORD`, `AGENT_AUTH_SECRET`) is **auto-generated by `start.sh` on first run** (#443, #589, #1159). On Linux, `DOCKER_GID` is auto-detected via `getent group docker`.
