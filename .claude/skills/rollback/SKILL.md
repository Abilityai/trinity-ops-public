...
**Rolling back across the ent#615 git token scrub is safe for existing agents.** Their remotes are already credential-less, and the `trinity` helper stays installed in their containers (`/etc/gitconfig` is not touched by a backend downgrade). Two things do change:
- The old backend writes token-bearing URLs again for *new* remote writes.
- An agent **recreated on a pre-0.9.5 base image** loses the helper. It then needs `GITHUB_PAT` in its `.env` or baked environment, which normal token propagation supplies.

If the base image is also rolled back, check `git fetch` on a couple of agents afterwards.
